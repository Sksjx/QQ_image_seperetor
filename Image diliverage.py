...
def Task(index:int,picture_list:list,cpu_count:int,detect_dic:list,current_num,setu_number,do_stop):
    ...
    while (task_round*cpu_count+index < total_num) and do_stop.value >= 0:
        ...
        if read_picture is None:
            try:
                os.rename(picture, folder_in+"/"+picture_list[task_round*cpu_count+index])
            except FileExistsError:
                print(picture+"正在被使用中")
            task_round += 1
            current_num.value += 1
            continue
        do_continue = True
        if detect_dic["Dectect_anime"] and do_continue:
            if detect_anime_face(old_net=anime_model,image_path=picture,gpu=gpu):
                do_continue = False
        if detect_dic["Dectect_humanface"] and do_continue:
            if detect_human_face(model=human_model,image_path=picture):
                do_continue = False
        if detect_dic["Dectect_entropy"] and not do_continue:
            if detect_entropy(image_path=picture):
                do_continue = True
        if not do_continue:
            setu_number.value += 1
            shutil.move(picture,folder_out)
            os.rename(folder_out+"/"+picture_name, folder_out+"/"+picture_list[task_round*cpu_count+index])
        task_round += 1
        current_num.value += 1
    if do_stop.value != -1:
        do_stop.value += 1
...
class main_window(object):
    def __init__(self):
        ...
        self.file_list = []
        self.detect_dic = {"Dectect_anime":False,"Dectect_humanface":False,"Dectect_entropy":False,"GPU":False,"folder_in":"","folder_out":"","anime_model_path":"","human_model_path":""}
    ...
    def BooleanVars(self):
        ...
        """
        self.Dectect_humanface = tkinter.BooleanVar()
    ...
    def Button(self):
        ...
        self.checkbutton_humanface = tkinter.Checkbutton(self.frame_checkbutton,text="启用人脸识别",variable=self.Dectect_humanface,onvalue=True,offvalue=False)
    ...
    def Run_Task(self):
        ...
        self.detect_dic["Dectect_humanface"] = self.Dectect_humanface.get()
        ...
        while (task_round*cpu_count < self.total_amount) and self.do_stop.value >= 0:
            ...
            if read_picture is None:
                try:
                    os.rename(picture, self.folder_in+"/"+self.file_list[task_round*cpu_count])
                except FileExistsError:
                    print(picture+"正在被使用中")
                task_round += 1
                ...
                continue
            do_continue = True
            if self.detect_dic["Dectect_anime"] and do_continue:
                if detect_anime_face(old_net=anime_model,image_path=picture,gpu=self.use_gpu.get()):
                    do_continue = False
            # print("anime finish")
            if self.detect_dic["Dectect_humanface"] and do_continue:
                if detect_human_face(model=human_model,image_path=picture):
                    do_continue = False
            # print("human finish")
            if self.detect_dic["Dectect_entropy"] and not do_continue:
                if detect_entropy(image_path=picture):
                    do_continue = True
            # print("entropy finish")
            if not do_continue:
                self.setu_number.value += 1
                shutil.move(picture,self.folder_out)
                os.rename(self.folder_out+"/"+picture_name, self.folder_out+"/"+self.file_list[task_round*cpu_count])
            self.current_num.value += 1
            task_round += 1
...
if __name__ =='__main__':
    ...
    # os.listdir(current_path+"/test")
    # image_path = current_path + '/test/1.jpg'
    # print(image_path)
    # model_path = current_path + '/model/ssd_anime_face_detect.pth'
